...
@broker.task
async def add_message(name: str, phone: str, bsuid:str, body: str, session: Annotated[AsyncSession, TaskiqDepends(get_session)]) -> str:
...
@broker.task
async def check_escalation(bsuid: str, session: Annotated[AsyncSession,TaskiqDepends(get_session)]) -> int:
...
@broker.task
async def get_response(user_id: int, session: Annotated[AsyncSession,TaskiqDepends(get_session)]) -> tuple[int, int]:
...
@broker.task
async def send_response(payload: tuple[int, int], session: Annotated[AsyncSession,TaskiqDepends(get_session)]):
